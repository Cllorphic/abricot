  ...
  return (
    <div className="border border-gray-200 rounded-2xl p-4 sm:p-5 bg-gray-50/50 lg:flex-1">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-xs bg-gray-200 text-gray-900 px-2 py-0.5 rounded-full">{tasks.length}</span>
      </div>
...
// ──── Barre de recherche ────
function SearchBar({ value, onChange, id = 'search-tasks' }) {
  return (
    ...
    <div className="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white w-full sm:max-w-lg sm:rounded-2xl rounded-t-2xl p-6 sm:p-8 relative max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-end gap-2 absolute top-4 right-4 sm:top-6 sm:right-6">
          ...
          <button onClick={onClose} className="text-gray-600 hover:text-gray-900 transition-colors" aria-label="Fermer">
            <Icon.Close />
          </button>
        </div>
        ...
        <form onSubmit={handleSave} className="flex flex-col gap-5 sm:gap-6">
          ...
          <div>
            ...
            <input id="task-title" type="text" className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm text-gray-900 placeholder-gray-600 bg-white focus:outline-none focus:border-orange-500" value={title} onChange={(e) => setTitle(e.target.value)} />
          </div>

          {/* Description */}
          <div>
            <label htmlFor="task-desc" className="block text-sm font-semibold text-gray-900 mb-2">Description</label>
            <textarea id="task-desc" className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm text-gray-900 placeholder-gray-600 bg-white focus:outline-none focus:border-orange-500 resize-none" rows={2} value={description} onChange={(e) => setDescription(e.target.value)} />
          </div>

          {/* Échéance */}
          <div>
            <label htmlFor="task-date" className="block text-sm font-semibold text-gray-900 mb-2">Échéance</label>
            <input id="task-date" type="date" className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm text-gray-900 bg-white focus:outline-none focus:border-orange-500" value={dueDate} onChange={(e) => setDueDate(e.target.value)} />
          </div>
              ...
              {['TODO', 'IN_PROGRESS', 'DONE'].map((s) => {
                ...
                return (
                  <button
                    ...
                    className={`text-sm px-5 py-2 rounded-full border transition-colors ${isActive ? `${c.bg} ${c.text} ${c.border} font-medium` : 'border-gray-300 text-gray-700 hover:border-gray-400'}`}
                  >
                    {c.label}
                  </button>
                );
          ...
          {/* Enregistrer */}
          <button type="submit" disabled={isSaving} className="bg-gray-100 text-orange-700 font-medium py-3 px-8 rounded-full hover:bg-gray-200 transition-colors disabled:opacity-50 w-fit">
...
function CreateProjectModal({ isOpen, onClose, authFetch }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white w-full sm:max-w-lg sm:rounded-2xl rounded-t-2xl p-6 sm:p-8 relative max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-4 right-4 sm:top-6 sm:right-6 text-gray-600 hover:text-gray-900" aria-label="Fermer"><Icon.Close /></button>
        <h2 className="text-xl font-bold text-gray-900 mb-6 sm:mb-8">Créer un projet</h2>
        {error && <p className="text-red-800 text-sm mb-4 p-3 bg-red-50 rounded-lg">{error}</p>}
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 sm:gap-6">
          <div>
            <label htmlFor="project-name" className="block text-sm font-medium text-gray-900 mb-2">Titre<span className="text-red-700">*</span></label>
            <input id="project-name" type="text" className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm text-gray-900 placeholder-gray-600 bg-white focus:outline-none focus:border-orange-500" value={name} onChange={(e) => setName(e.target.value)} placeholder="Nom du projet" />
          </div>
          <div>
            <label htmlFor="project-desc" className="block text-sm font-medium text-gray-900 mb-2">Description<span className="text-red-700">*</span></label>
            <textarea id="project-desc" className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm text-gray-900 placeholder-gray-600 bg-white focus:outline-none focus:border-orange-500 resize-none" rows={3} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Description du projet" />
          </div>
          <div>
          ...
          <button type="submit" disabled={isSubmitting} className="bg-gray-100 text-orange-700 font-medium py-3 px-6 rounded-full hover:bg-gray-200 transition-colors disabled:opacity-50 w-fit">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 py-6 sm:py-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6 sm:mb-8">
          <div>
            ...
            <p className="text-sm sm:text-base text-gray-700 mt-1">Bonjour {user?.name || user?.email}, voici un aperçu de vos projets et tâches</p>
          </div>
        ...
        <div className="flex items-center gap-1 mb-6 overflow-x-auto" role="tablist" aria-label="Vues du tableau de bord">
          {[
            ...
            <button key={tab.key} role="tab" aria-selected={view === tab.key} onClick={() => setView(tab.key)} className={`flex items-center gap-2 text-sm px-4 py-2 rounded-full transition-colors whitespace-nowrap ${view === tab.key ? 'bg-orange-50 text-orange-700 font-medium' : 'text-gray-700 hover:bg-gray-100'}`}>
        ...
        {view === 'list' && (
          <div className="border border-gray-200 rounded-2xl p-4 sm:p-6" role="tabpanel">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
              ...
              <SearchBar id="search-tasks-list" value={search} onChange={setSearch} />
            </div>
            {loadingData ? (
              <p className="text-gray-700 text-center py-12">Chargement...</p>
            ) : listTasks.length === 0 ? (
              <p className="text-gray-700 text-center py-12">{search ? 'Aucune tâche trouvée' : 'Aucune tâche assignée'}</p>
            ) : (
        ...
        {view === 'kanban' && (
          <div role="tabpanel">
            <div className="mb-4"><SearchBar id="search-tasks-kanban" value={search} onChange={setSearch} /></div>
